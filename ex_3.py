...
j_nll = lambda v, y_correct: -np.log(v.T[y_correct])


def soft_max_prime(softmax):
...
def train(x_data, y_data, ephocs=10):
    ...
    funcs = {0: sigmoid, 1: sigmoid, 2: sigmoid, 3: soft_max}
    div_funcs = {0: sigmoid_prime, 1: sigmoid_prime, 2: sigmoid_prime, 3: soft_max_prime}
    ...
    w1 = np.random.rand(x_data.shape[1], num_of_nurons_1)
    w2 = np.random.rand(num_of_nurons_1, num_of_nurons_2)
    w3 = np.random.rand(num_of_nurons_2, num_of_nurons_3)
    w4 = np.random.rand(num_of_nurons_3, number_of_labels)
    ...
    for _ in range(ephocs):
        ...
        for x, y in zip(x_data, y_data):
            ...
            dz3 = np.dot(np.dot(dz4, w4.T), div_funcs[3](z3))
            dw3 = np.dot(g2.T, dz3)

            dz2 = np.dot(dz3, w3.T) * div_funcs[2](z2)
            dw2 = np.dot(g1.T, dz2)

            dz1 = np.dot(dz2, w2.T) * div_funcs[1](z1)
            dw1 = np.dot(x, dz1)
